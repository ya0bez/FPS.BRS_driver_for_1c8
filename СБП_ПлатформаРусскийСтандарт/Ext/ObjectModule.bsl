...
Функция ОплатитьПлатежнойКартой(ИДУстройства, НомерКарты = "", Сумма, НомерЧека = "",
	...
	Если ПараметрыДрайвера.ТипОплаты = 1 Тогда // Оплата динамическим QR-кодом
		Попытка
			РезультатОДК = ОплатитьДинамическимКодом(Сумма);
		Исключение
			ВыполнитьЗаписьЖурналаРегистрацииПриОшибки(ОписаниеОшибки());
			ВыполнитьЗаписьЛогаПриОшибки(ОписаниеОшибки());
			Возврат Ложь;
	...
	ИначеЕсли ПараметрыДрайвера.ТипОплаты = 2 Тогда // Оплата кассовой платежной ссылкой
		Попытка
			РезультатОКС = ОплатитьКассовойПлатежнойСсылкой(Сумма);
		Исключение
			ВыполнитьЗаписьЖурналаРегистрацииПриОшибки(ОписаниеОшибки());
			ВыполнитьЗаписьЛогаПриОшибки(ОписаниеОшибки());
			Возврат Ложь;
...
Функция ВернутьПлатежПоПлатежнойКарте(ИДУстройства, НомерКарты = "", Сумма, НомерЧека = "",
	...
	Попытка
		РезультатВозвртОплаты = ВыполнитьВозвратОплаты(Сумма, КодRRN, ИДУстройства);
	Исключение
		ВыполнитьЗаписьЖурналаРегистрацииПриОшибки(ОписаниеОшибки());
		ВыполнитьЗаписьЛогаПриОшибки(ОписаниеОшибки());
		Возврат Ложь;
	КонецПопытки;
	ПараметрыЛогирования = КонструкторПараметровЗаписиЛогирования(
			"Результат проведения возврата оплаты СБП",
			Новый Структура("Сумма", Сумма),
			ПолучитьКопиюСтруктурыПоВключаемымКлючам(РезультатВозвртОплаты, "Результат"));
	ВыполнитьЗаписьЛога( , ПараметрыЛогирования);
	Если НЕ РезультатВозвртОплаты.Результат Тогда
		УстановитьОшибку(КодОперации, Ложь, РезультатВозвртОплаты.Комментарий);
		Возврат Ложь;
	КонецЕсли;
	
	КодАвторизации = РезультатВозвртОплаты.КодАвторизации;
	ТекстКвитанции = РезультатВозвртОплаты.ТекстКвитанции;
	КодRRN = РезультатВозвртОплаты.КодRRN;
	НомерЧека = РезультатВозвртОплаты.НомерЧека;
	Возврат Истина;
...
Функция ОповещениеОтменыОплатыQR(РезультатВыполнения, Знач Параметры) Экспорт
	...
	РезультатВыполнения.Вставить("Комментарий", "Отмена оплаты.");
	...
	Если СтрСравнить(Параметры.МетодОплаты, "КПС") = 0 Тогда
		...
		// Выполним деактивацию ссылки.
		РезультатДеактивацияКассовойПлатежнойСсылки = ДеактивацияКассовойПлатежнойСсылки();
		ЗаполнитьЗначенияСвойств(РезультатВыполнения, РезультатДеактивацияКассовойПлатежнойСсылки, , "Комментарий");
		РезультатВыполнения.Комментарий = СтрШаблон("%1 %2",
				РезультатВыполнения.Комментарий, РезультатДеактивацияКассовойПлатежнойСсылки.Комментарий);
		Возврат РезультатВыполнения;
	ИначеЕсли СтрСравнить(Параметры.МетодОплаты, "DQR") Тогда
		// Ветка отмены оплаты по динамической ссылке
		РезультатВыполнения.Комментарий = СтрШаблон("%1 Для текущего метода оплаты (%2) отмена не поддерживается.",
				РезультатВыполнения.Комментарий, Параметры.МетодОплаты);
		Возврат РезультатВыполнения;
	Иначе
		РезультатВыполнения.Комментарий = СтрШаблон("%1 Неизвестный метод оплаты (%2).",
				РезультатВыполнения.Комментарий, Параметры.МетодОплаты);
		Возврат РезультатВыполнения;
	КонецЕсли;
...
Функция ВыполнитьВозвратОплаты(Знач Сумма, Знач ИдентификаторыВозврата, Знач ИДУстройства = Неопределено)
	
	РезультатВыполнения = Новый Структура;
	РезультатВыполнения.Вставить("Результат", Ложь);
	РезультатВыполнения.Вставить("Комментарий", "Возврат оплаты СБП.");
	...
	Если НЕ ТипЗнч(Сумма) = Тип("Число")
		ИЛИ Сумма <= 0 Тогда
		РезультатВыполнения.Комментарий = СтрШаблон("%1 %2",
				РезультатВыполнения.Комментарий, "Сумма возврата отсутствует или отличается от ожидаемой.");
		Возврат РезультатВыполнения;
	КонецЕсли;
	...
	Если ПустаяСтрока(ИдентификаторыВозврата) Тогда
		...
		Возврат РезультатВыполнения;
	КонецЕсли;
	ИдентификаторыВозврата = СтрРазделить(ИдентификаторыВозврата, ":", Истина);
	...
	Если ПустаяСтрока(РегистрационныйНомерЮЛ) Тогда
		РезультатВыполнения.Комментарий = СтрШаблон("%1 %2",
				РезультатВыполнения.Комментарий, "Отсутствует регистрационный номер ЮЛ (UserLogin).");
		Возврат РезультатВыполнения;
	КонецЕсли;
	...
	Если ПустаяСтрока(АдресРесурсаЗапросаВозврата) Тогда
		РезультатВыполнения.Комментарий = СтрШаблон("%1 %2",
				РезультатВыполнения.Комментарий, "Отсутствует адрес ресурса запроса возврата.");
		Возврат РезультатВыполнения;
	КонецЕсли;
	...
	Если ПустаяСтрока(АдресРесурсаЗапросаПроверкиВозврата) Тогда
		РезультатВыполнения.Комментарий = СтрШаблон("%1 %2",
				РезультатВыполнения.Комментарий, "Отсутствует адрес ресурса запроса проверки статуса возврата.");
		Возврат РезультатВыполнения;
	КонецЕсли;
	...
	Если ПустаяСтрока(ИдентификаторТСП) Тогда
		РезультатВыполнения.Комментарий = СтрШаблон("%1 %2",
				РезультатВыполнения.Комментарий, "Отсутствует идентификатор ТСП (merchantid).");
		Возврат РезультатВыполнения;
	КонецЕсли;
	...
	Если ПараметрыДрайвера.Свойство("БезопасностьOpenSSL") Тогда
		OpenSSL = ПараметрыДрайвера.БезопасностьOpenSSL;
	КонецЕсли;
	Если ПустаяСтрока(OpenSSL)
		ИЛИ OpenSSL = Неопределено Тогда
		РезультатВыполнения.Комментарий = СтрШаблон("%1 %2",
				РезультатВыполнения.Комментарий, "Отсутствует путь к исполняемому файлу OpenSSL.");
		Возврат РезультатВыполнения;
	КонецЕсли;
	...
	Если ПараметрыДрайвера.Свойство("БанкСертификатЭЦП") Тогда
		БанкСертификатЭЦП = ПараметрыДрайвера.БанкСертификатЭЦП;
	КонецЕсли;
	Если ПустаяСтрока(БанкСертификатЭЦП)
		ИЛИ БанкСертификатЭЦП = Неопределено Тогда
		РезультатВыполнения.Комментарий = СтрШаблон("%1 %2",
				РезультатВыполнения.Комментарий, "Отсутствует файл сертификата ЭЦП.");
		Возврат РезультатВыполнения;
	КонецЕсли;
	...
	Если НЕ РезультатОжиданиеВозврата.Результат Тогда
		РезультатВыполнения.Комментарий = СтрШаблон("%1 %2 %3",
				РезультатВыполнения.Комментарий, "Ошибка выполнения:", РезультатОжиданиеВозврата.Комментарий);
		Возврат РезультатВыполнения;
	КонецЕсли;
	...
	РезультатВыполнения.КодRRN = КодRRN;
...
Функция ОплатитьДинамическимКодом(Знач Сумма)
...
Функция ОплатитьКассовойПлатежнойСсылкой(Знач Сумма)
...
Функция ВыполнитьВыводНаДисплей(Параметры = Неопределено)
	...
	РезультатВыполнения = Новый Структура;
	РезультатВыполнения.Вставить("Результат", Ложь);
	РезультатВыполнения.Вставить("Комментарий", "QR-дисплей. Включение\Выключение\Вывод информации. %1");
...
Функция СведенияОПрограмме()
	...
	ОписаниеОбработки.Вставить("ДатаСоздания", Формат('2022-03-29', "ДФ=dd.MM.yyyy"));
	...
	ОписаниеОбработки.ОписаниеВерсий.Добавить(
		КонструкторОписаниеВерсийПрограммы("1.1.1", '2022-03-29', "КЮБ",
			"Реализована механика работы с Системой Быстрых платежей через реализацию банка Русский Стандарт"));
	
	ОписаниеОбработки.ОписаниеВерсий.Добавить(
		КонструкторОписаниеВерсийПрограммы("1.2.1", '2022-04-08', "КЮБ"));
	
	ОписаниеОбработки.ОписаниеВерсий.Добавить(
		КонструкторОписаниеВерсийПрограммы("1.2.2", '2022-04-18', "КЮБ",
			"Доработка от 18.04.2022"));
	
	ОписаниеОбработки.ОписаниеВерсий.Добавить(
		КонструкторОписаниеВерсийПрограммы("1.2.3", '2022-04-20', "КЮБ",
			"Доработка от 20.04.2022. Добавлена возможность печати QR на ККМ"));
	
	ОписаниеОбработки.ОписаниеВерсий.Добавить(
		КонструкторОписаниеВерсийПрограммы("1.2.4", '2022-06-21', "КЮБ",
			"Доработка от 21.06.2022. Доработана возможность не выводить графический код на устройство"));
	
	ОписаниеОбработки.ОписаниеВерсий.Добавить(
		КонструкторОписаниеВерсийПрограммы("1.2.5", '2022-06-22', "КЮБ",
			"Доработка от 22.06.2022. Доработка функционала, устранение ошибок программы"));
	
	ОписаниеОбработки.ОписаниеВерсий.Добавить(
		КонструкторОписаниеВерсийПрограммы("1.3.1", '2022-06-23', "КЮБ",
	...
	ОписаниеОбработки.ОписаниеВерсий.Добавить(
		КонструкторОписаниеВерсийПрограммы("1.5.2", '2023-08-10', "КЮБ",
			"Доработка от 10.08.2023. Доработка логирования, общая доработка функционала"));
	
	ОписаниеОбработки.ОписаниеВерсий.Добавить(
		КонструкторОписаниеВерсийПрограммы("1.5.3", '2023-08-14', "КЮБ",
			"Доработка от 14.08.2023. Исправление ошибки с формированием комментария."));
	
	ОписаниеОбработки.ОписаниеВерсий.Добавить(
		КонструкторОписаниеВерсийПрограммы("1.5.4", '2023-08-15', "КЮБ",
			"Доработка от 15.08.2023. Общие доработки."));
	
	ОписаниеОбработки.ОписаниеВерсий.Добавить(
		КонструкторОписаниеВерсийПрограммы("1.5.5", '2023-08-20', "КЮБ",
			"Доработка от 20.08.2023. Общие доработки. Фикс ошибок."));
	
	ОписаниеОбработки.ОписаниеВерсий.Добавить(
		КонструкторОписаниеВерсийПрограммы("1.5.6", '2023-08-21', "КЮБ", "Доработка от 21.08.2023. Общие доработки."));
	
	ОписаниеОбработки.ОписаниеВерсий.Добавить(
		КонструкторОписаниеВерсийПрограммы("1.5.7", '2023-08-24', "КЮБ", "Доработка от 24.08.2023. Общие доработки."));
	
	ОписаниеОбработки.ОписаниеВерсий.Добавить(
		КонструкторОписаниеВерсийПрограммы("1.5.9", '2023-08-28',
			"КЮБ", "Доработка от 28.08.2023. Доработки логирования системы. Общие доработки."));
	
	ОписаниеОбработки.ОписаниеВерсий.Добавить(
		КонструкторОписаниеВерсийПрограммы("1.5.10", '2023-08-29', "КЮБ", "Доработка от 29.08.2023."));
	
	ОписаниеОбработки.ОписаниеВерсий.Добавить(
		КонструкторОписаниеВерсийПрограммы("1.5.11", '2023-08-31', "КЮБ", "Доработка от 31.08.2023. Фиксация ошибки."));
	
	ОписаниеОбработки.ОписаниеВерсий.Добавить(
		КонструкторОписаниеВерсийПрограммы("1.5.12", '2023-09-01', "КЮБ", "Доработка от 01.09.2023 Фиксация ошибки."));
	
	ОписаниеОбработки.ОписаниеВерсий.Добавить(
		КонструкторОписаниеВерсийПрограммы("1.5.14", '2023-09-05', "КЮБ", "Доработка от 05.09.2023."));
